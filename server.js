...
ענה במספר בלבד.`);
      ...
      case 'ask_age':
        ...
        user = await refreshUser(phone);
        twiml.message('מה המשקל שלך בק"ג?');
        break;
      ...
      case 'ask_weight':
        ...
        twiml.message('מה הגובה שלך בס"מ?');
        break;
...
${byGender(user, 'ענה', 'עני')} במספר בלבד.`);
        }
        break;
...
כמה זמן לקח לך בפועל?
${byGender(user, 'תכתוב', 'תכתבי')} מספר בדקות בלבד.`);
          break;
        }
...
5 - קשה מאוד`
...

        {
          ...
          let easyStreak = user.easy_streak || 0;
          let nextLevel = user.current_level;
          let nextWorkoutIndex = user.current_workout_index + 1;
          ...
          let threshold = 3;
          if (user.fitness_level === 2) threshold = 2;
          if (user.fitness_level === 3) threshold = 2;

          if (easyStreak >= threshold && nextLevel < 3) {
            nextLevel += 1;
            easyStreak = 0;
          }
          ...
          await pool.query(
            `UPDATE users
             SET current_workout_index = $1,
                 ...
                 pending_actual_minutes = NULL
             WHERE id = $4`,
            [nextWorkoutIndex, nextLevel, easyStreak, user.id]
          );
